...
public:
  ADC_DMA(ADC_HandleTypeDef *_handle, const int _NumOfChannel, bool chalibration = true)
      : handle(_handle), NumOfChannel(_NumOfChannel) {
  ...
  void startDMA() { HAL_ADC_Start_DMA(handle, (uint32_t *)&buf[0], 1); }
};
